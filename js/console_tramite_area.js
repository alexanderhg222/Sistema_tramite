var tbl_tramite_area;
function listar_tramite_area() {
  let idusaurio = document.getElementById("txt_id_sesion_area").value;
  console.log(idusaurio);
  tbl_tramite_area = $("#tabla_tramite").DataTable({
    ordering: false,
    bLengthChange: true,
    searching: false,
    paging: false,
    ...
    processing: true,
    ajax: {
      url: "../Controller/tramite/controlador_listar_tramite_sesion.php",
      type: "POST",
      data: {
        idusaurio,
      },
...
function pruebaseguimineto() {
  $("#tabla_tramite").on("click", ".seguimiento", function () {
    var data = tbl_tramite_area.row($(this).parents("tr")).data();
    //en tamaño escritorio
    if (tbl_tramite_area.row(this).child.isShown()) {
      var data = tbl_tramite_area.row(this).data();
    }
...
function pruebasderivacion() {
  $("#tabla_tramite").on("click", ".derivacion", function () {
    ...
    document.getElementById("select_origen").value = data.destino;
    document.getElementById("txt_id_doc").value = data.idocumento_id;
  });
}
...
function Registrar_derivacion() {
  let select_origen = document.getElementById("txt_id_sesion_area").value;
  let select_destino = document.getElementById("select_destino").value;
  let txt_id_doc = document.getElementById("txt_id_doc").value;
  let arc = document.getElementById("txt_documento").value;
  let txt_descripcion = document.getElementById("txt_descripcion").value;
  let idusaurio = document.getElementById("txt_id_sesion_usu").value;
  ...
  let formData = new FormData();
  let archivoobj = $("#txt_documento")[0].files[0]; //El objeto del archivo adjuntado
  //////DATOS DEL REMITENTE

  formData.append("select_origen", select_origen);
  formData.append("select_destino", select_destino);
  formData.append("txt_id_doc", txt_id_doc);
  formData.append("txt_descripcion", txt_descripcion);
  formData.append("idusaurio", idusaurio);
  formData.append("nombrearchivo", nombrearchivo);
  formData.append("archivoobj", archivoobj);
  $.ajax({
    url: "../controller/tramite/controlador_registro_derivacion.php",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
